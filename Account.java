...
public interface MiniSocialMediaPlatform extends Serializable {
	...
	void changeAccountHandle(String oldHandle, String newHandle)
			throws HandleNotRecognisedException, IllegalHandleException, InvalidHandleException {
				...
				for (int i = 0; i < accountArrayList.size(); i++) {
					...
					} else if (((((accountArrayList.get(i)).getHandle()).equals(this.oldHandlehandle)) == false) && i == accountArrayList.size()){
	...
	String showAccount(String handle) throws HandleNotRecognisedException; {
		...
		for (int i = 0; i < accountArrayList.size(); i++) {
			if (((accountArrayList.get(i)).getHandle()).equals(this.handle)) {
				String output = String.format("ID: %s /n Handle: %s /n Description: %s /n Post Count: %s /n Endorsed Posts: %s /n",accountArrayList.get(i)).getAccountId(),(accountArrayList.get(i)).getHandle(),accountArrayList.get(i)).getDescription());
